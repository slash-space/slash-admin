...
func NewCaptchaStoreWithRedis(r *redis.Redis) *RedisStore {
	return &RedisStore{
...
func (r *RedisStore) Set(id string, value string) error {
	...
	if err != nil {
		logx.Error("captcha: RedisStoreSet Error!", err)
		return err
	}
	return nil
}
...
func (r *RedisStore) Get(key string, clear bool) string {
	...
	if err != nil {
		logx.Error("captcha: RedisStoreGet Error!", err)
		return ""
	}
	if clear {
		_, err := r.Redis.Del(key)
		if err != nil {
			logx.Error("captcha: RedisStoreClear Error!", err)
			return ""
		}
	}
	return val
